...
举例：当前已经推理出来的seq是 [12, 34, 178, 19], 这里用一个整数表示vocabulary里的一个token id。在预测新的token时，将19的 $Q$ 跟 $12, 34, 178$ 的 $K/V$ 做运算。假定新的token的id是 17， 我们得到seq 为 [12, 34, 178, 19, 17]。 继续预测下一个token时，我们需要将17的 $Q$ 跟 $12, 34, 178, 19$ 的 $K/V$ 做运算。显然，token $12, 34, 178$ 的 $K/V$ 需要重复计算两次。

由此， 我们可以提前将seq里的每个token的 $K/V$ 保存起来，每生成一个新的token，就将这个新token的 $K/V$ 加到前面token的缓存中。预测下一个token的时候，就可以直接拿来用了。
