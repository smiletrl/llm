...
- 角度的计算规则，以 $Q$ 为例。它的最后一个维度，拆分为两两一对向量，比如 $[X_1, X_2, X_3, \dots, X_n]$，被拆成 $n / 2$ 对，$[[X_1, X_{n/2}], [X_2, X_{n/2+1}], \dots, [X_{n/2-1}, X_n]]$。这是参照 Meta LLaMA 风格，也是目前通用的标准。在[苏剑林的原始 RoFormer 论文](https://arxiv.org/abs/2104.09864)中，采用的是相邻维度拆分，也就是 $[[X_1, X_2], [X_3, X_4], \dots]$。拆分后每对向量的旋转有不同的频率。
